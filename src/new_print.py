...
class InvoiceGenerator:
    ...
    def _validate_data(self, data):
        ...
        if not data["items"] or not all(
            isinstance(item, dict)
            and all(k in item for k in ["name", "brand", "number", "price", "quantity"])
    ...
    def create_invoice(
        self, data, output_mode="file", filename="invoice.docx"
    ) -> str | BytesIO:
        try:
            ...
            if data["total"] != calc_total:
                print(
                    f"Предупреждение: Итоговая сумма не совпадает. Использую {calc_total}"
                )
            ...
            self.doc.add_paragraph(
                f"Дата: {datetime.datetime.now().strftime(DATE_FORMAT)}"
            )
...
try:
    result = generator.create_invoice(
        invoice_data, output_mode="file", filename="invoice.docx"
    )
    print(result)
except Exception as e:
    print("Ошибка:", e)
