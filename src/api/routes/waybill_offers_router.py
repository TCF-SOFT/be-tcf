...
router = APIRouter(tags=["Waybill-Offers"], prefix="/waybill-offers", dependencies=[Depends(require_employee)])
...
async def delete_offer_from_waybill(
    ...
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Offer not found in waybill")
    return
